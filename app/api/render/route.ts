      ...
      if (strokes?.strokes.length) {
        ...
        if (hasPencil && scene.effects.marker) {
          await runFfmpeg([
            ...
            "-filter_complex",
            `[1:a]volume=0.45,afade=t=out:st=${Math.max(0.2, drawSec - 0.25)}:d=0.25[pencil];[2:a][pencil]amix=inputs=2:duration=first:dropout_transition=0[a]`,
            "-map", "0:v",
            "-map", "[a]",
            "-t", String(scene.duration),
            "-c:v", "libx264",
        ...
        await runFfmpeg([
          ...
          "-i", pencilSound,
          "-filter_complex",
          `[0:v]settb=AVTB[base];[1:v]scale=1000:1500:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2:color=f9f8f2,fps=30,settb=AVTB,format=yuv420p[ink];[base][ink]xfade=transition=wipeleft:duration=${drawSec}:offset=0,format=yuv420p[v];[2:a]volume=0.4,afade=t=out:st=${Math.max(0.2, drawSec - 0.2)}:d=0.2,apad=whole_dur=${scene.duration}[a]`,
          "-map", "[v]",
          "-map", "[a]",
          "-t", String(scene.duration),
          "-c:v", "libx264",
